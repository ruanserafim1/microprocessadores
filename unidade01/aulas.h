...
GPIOA -> MODER |= (0b01 << 2) | (0b01 << 6) | (0b01 << 10); // conf. os pino PA2, PA3 e PA5 como saída usando o 0b01.
  while(1){
  for(int i =0;i < 3; i++){
    GPIOA->ODR |=(1 << 1);  //  liga PA1
    GPIOA->ODR &= ~((1 << 3) | (1 << 5)); // desliga PA3 e PA5
    Delay_ms(2000);
    GPIOA->ODR |= (1 << 3); //liga PA3
    GPIOA->ODR &= ~((1 << 1) | (1 << 5)); // desliga PA1 E PA5
    Delay_ms(1000);
    GPIOA->ODR |= (1 << 5);  // liga PA5
    GPIOA->ODR &= ~((1 << 1) | (1 << 3)); // desliga PA1 E PA3
    Delay_ms(2000);
    GPIOA->ODR &= ~(1 << 5); // desliga PA5
}

// AMARELO PISCANTE (PISCAR PA3)
...
while(1){
  ...
  Delay_us(1000-contador);
if (bool){
  contador++;
  bool = contador == 1000 ? 0 : 1;
}
else 
  contador--;
  bool = contador == 0 ? 1 : 0;
...
11= não aceita
*/
GPIOE ->PUPDR  |= (0b01 << 8); // liga resistor em pull up em PE4
...
void questao10(void){
RCC->AHB1ENR |= 1;           // liga o clock do GPIOA
GPIOA->MODER |= (0b01 << 12) | (0b01 << 14); // pino PA6 como saída
int contador = 0;
int bool = 1;
while(1){
  GPIOA->ODR &= ~(1 << 6);      //nível baixo para o pino
